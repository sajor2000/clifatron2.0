...
| **Vocabulary / Binning** | Clinical-reference-range segments (lactate: 15 bins, MAP: 25 bins, 1268 segments total from the CLIF consortium CSV) | **Same 1268-physician-designed-bin CSV KE PT from v1** as the primary path, with three additions: soft discretization (Gaussian-weight to adjacent bins), fused `code=bin` tokens (one token per event), and forced clinical-threshold edges. Population deciles retained as the `decile_ablation` arm only | The clinical-expert bins are preserved — they encode measurement-density domain expertise that data-driven deciles cannot recover. v2 adds soft smoothing and fusion but does not replace your bin design |
| **Tokenization** | Fused `concept=bin` tokens (per-event: `labs_lactate_(2.0,2.2]`) via tokenETL's `bin_numeric_values_with_intervals_by_category` | **Same fused `code=bin` format KE PT**, plus soft discretization (Gaussian spread to ±1 neighbor bin), admission-relative RoPE position encoding (replacing inserted `day_N`/`hour_N` time tokens), and `build_edges` dispatch for scheme-agnostic binning | Format preserved for checkpoint compatibility. Soft discretization adds boundary sensitivity where hard bin edges lose information. RoPE cuts sequence length ~11% and transports across hospitals |
