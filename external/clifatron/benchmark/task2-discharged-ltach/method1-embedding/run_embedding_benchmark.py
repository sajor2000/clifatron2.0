#!/usr/bin/env python3
"""
Method 1: Embedding-based Benchmark for Task 1: Discharged Home Prediction
...
4. Predicts binary outcome: discharged home (yes/no)
...
def train_classifier(
    ...
    classifier = xgb.XGBClassifier(
        ...
        scale_pos_weight=clf_config.get("scale_pos_weight", scale_pos_weight),
        eval_metric="logloss",
        random_state=42,
        n_jobs=-1,
    )
...
def load_embeddings(cache_path: Path, data_path: Path, label_column: str = "label_ltach") -> tuple[np.ndarray, np.ndarray]:
    """
    ...
    Args:
        ...
        label_column: Name of label column in data file (default: "label_ltach" for Task2)
...
def main():
    ...
    if args.output_dir is None:
        results_writer = get_results_writer(
            task_name="task2-discharged-ltach",
    ...
    if is_main_process():
        ...
        try:
            ...
            train_data_path = input_dir / "task1_task2_disposition_train_val.parquet"
            test_data_path = input_dir / "task1_task2_disposition_test.parquet"

            # Load embeddings and labels from cache + data files
            train_embeddings, train_labels = load_embeddings(
                train_cache_path,
                train_data_path,
                label_column="label_ltach"  # Task2: LTACH Discharge
            )
    ...
    if is_main_process():
        ...
        try:
            ...
            metrics = compute_metrics(
                ...
                y_prob=eval_results["probabilities"],
            )
